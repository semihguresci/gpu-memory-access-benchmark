...
EXPERIMENT_ID = "42_async_compute_overlap"
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"] == "serial_no_overlap"][
    ...
    relative["slowdown_vs_serial"] = relative["gpu_ms_median"] / relative["baseline_gpu_ms_median"]
    relative["throughput_ratio_vs_serial"] = relative["throughput_median"] / relative["baseline_throughput_median"]
    relative["gbps_ratio_vs_serial"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "problem_size",
            ...
            "slowdown_vs_serial",
            "throughput_median",
            "throughput_ratio_vs_serial",
            "gbps_median",
            "gbps_ratio_vs_serial",
        ]
    ]
...
def main() -> None:
    ...
    _write_table(summary, "async_compute_overlap_summary.csv")
    _write_table(relative, "async_compute_overlap_relative.csv")
    _write_table(stability, "async_compute_overlap_stability.csv")
    print(
