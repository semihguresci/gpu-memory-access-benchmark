...
def main() -> None:
    summary = _load("frame_to_frame_coherence_summary.csv")
    relative = _load("frame_to_frame_coherence_relative.csv")
    stability = _load("frame_to_frame_coherence_stability.csv")
    ...
    ax.set_title("Experiment 37: Sequence GPU Time by Problem Size")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("GPU ms (median)")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "frame_to_frame_coherence_ms.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_title("Experiment 37: Sequence Throughput by Problem Size")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("throughput (elements/s, median)")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "frame_to_frame_coherence_throughput.png", dpi=150)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(10, 6))
    for variant in relative["variant"].unique():
        subset = relative[relative["variant"] == variant].sort_values("problem_size")
        ax.plot(subset["problem_size"], subset["slowdown_vs_coherent"], marker="o", linewidth=2.0, label=variant)
    ax.axhline(1.0, color="black", linestyle="--", linewidth=1.0)
    ax.set_title("Experiment 37: Slowdown vs coherent_shift")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("slowdown ratio")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "frame_to_frame_coherence_relative.png", dpi=150)
    plt.close(fig)
    ...
    ax.axhline(1.0, color="black", linestyle="--", linewidth=1.0)
    ax.set_title("Experiment 37: Stability by Problem Size")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("p95 / median GPU ms")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "frame_to_frame_coherence_stability.png", dpi=150)
    plt.close(fig)

    print(f"[ok] Wrote Experiment 37 charts to {CHARTS_DIR}.")
