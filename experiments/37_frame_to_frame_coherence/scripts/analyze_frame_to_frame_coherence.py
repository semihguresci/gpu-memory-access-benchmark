...
EXPERIMENT_ID = "37_frame_to_frame_coherence"
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"] == "coherent_shift"][
    ...
    relative["slowdown_vs_coherent"] = relative["gpu_ms_median"] / relative["baseline_gpu_ms_median"]
    relative["throughput_ratio_vs_coherent"] = (
        relative["throughput_median"] / relative["baseline_throughput_median"]
    )
    relative["gbps_ratio_vs_coherent"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "problem_size",
            "gpu_ms_median",
            "slowdown_vs_coherent",
            "throughput_median",
            "throughput_ratio_vs_coherent",
            "gbps_median",
            "gbps_ratio_vs_coherent",
        ]
    ]
...
def main() -> None:
    ...
    _write_table(summary, "frame_to_frame_coherence_summary.csv")
    _write_table(relative, "frame_to_frame_coherence_relative.csv")
    _write_table(stability, "frame_to_frame_coherence_stability.csv")
    print(
