...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Experiment 27 JSON output into runs/<device>/...")
    parser.add_argument(
        "--input",
        type=Path,
        ...
        help=(
            ...
            "(default: experiments/27_cache_thrashing_random_vs_sequential/results/tables/benchmark_results.json)."
        ),
    )
    parser.add_argument(
        "--runs-dir",
        ...
        help="Destination runs directory (default: experiments/27_cache_thrashing_random_vs_sequential/runs).",
    )
    parser.add_argument(
        "--label",
        type=str,
    ...
    if not any(str(row.get("experiment_id", "")) == EXPERIMENT_ID for row in rows):
        raise ValueError(
            "Input JSON has rows but no Experiment 27 entries. "
            "Run with --experiment 27_cache_thrashing_random_vs_sequential first."
        )
