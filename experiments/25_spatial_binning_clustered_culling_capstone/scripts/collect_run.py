...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Experiment 25 JSON output into runs/<device>/...")
    ...
    if not any(str(row.get("experiment_id", "")) == EXPERIMENT_ID for row in rows):
        raise ValueError(
            "Input JSON has rows but no Experiment 25 entries. "
            "Run with --experiment 25_spatial_binning_clustered_culling_capstone first."
        )
