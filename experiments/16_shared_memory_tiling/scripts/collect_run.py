...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Experiment 16 JSON output into runs/<device>/...")
    parser.add_argument(
        "--input",
        type=Path,
        ...
        help=(
            ...
            "(default: experiments/16_shared_memory_tiling/results/tables/benchmark_results.json)."
        ),
    )
    parser.add_argument(
        "--runs-dir",
        ...
        help="Destination runs directory (default: experiments/16_shared_memory_tiling/runs).",
    )
    parser.add_argument(
        "--label",
        type=str,
    ...
    if not any(str(row.get("experiment_id", "")) == EXPERIMENT_ID for row in rows):
        raise ValueError(
            "Input JSON has rows but no Experiment 16 entries. "
            "Run with --experiment 16_shared_memory_tiling first."
        )
