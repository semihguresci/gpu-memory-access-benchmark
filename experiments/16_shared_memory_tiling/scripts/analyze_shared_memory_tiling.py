...
EXPERIMENT_ID = "16_shared_memory_tiling"
...
def _load_frame(input_path: Path) -> tuple[pd.DataFrame, dict]:
    ...
    if frame.empty:
        raise ValueError("Input benchmark JSON has rows but no Experiment 16 entries.")
    ...
    for column_name, note_key in [
        ...
        ("reuse_radius", "reuse_radius"),
        ("center_offset", "center_offset"),
        ("local_size_x", "local_size_x"),
        ("group_count_x", "group_count_x"),
        ("source_padded_elements", "source_padded_elements"),
        ("output_elements", "output_elements"),
        ("active_tile_span_elements", "active_tile_span_elements"),
    ...
    frame["reuse_radius"] = pd.to_numeric(
        frame["reuse_radius"].fillna(frame["variant"].str.extract(r"_r(\d+)$")[0]),
        errors="coerce",
    )
    ...
    for column_name, default_value in [
        ...
        ("local_size_x", 256),
        ("group_count_x", 0),
        ("source_padded_elements", 0),
        ("output_elements", 0),
        ("active_tile_span_elements", 0),
    ...
    if frame["reuse_radius"].isna().any():
        raise ValueError("Could not determine reuse_radius for one or more rows.")

    frame["reuse_radius"] = frame["reuse_radius"].astype(int)
    ...
    frame["source_padded_elements"] = frame["source_padded_elements"].astype(int)
    frame["output_elements"] = frame["output_elements"].replace(0, pd.NA).fillna(frame["problem_size"]).astype(int)
    frame["active_tile_span_elements"] = frame["active_tile_span_elements"].astype(int)
...
def _load_source_frame(skip_current: bool) -> tuple[pd.DataFrame, dict, Path]:
    ...
    if latest_run is None:
        ...
        raise FileNotFoundError(
            "No benchmark_results.json or archived run JSON found for Experiment 16. "
...
def _build_summary(frame: pd.DataFrame) -> pd.DataFrame:
    summary = (
        frame.groupby(["implementation", "reuse_radius", "logical_elements"], as_index=False)
        .agg(
            ...
            source_padded_elements=("source_padded_elements", "first"),
            output_elements=("output_elements", "first"),
            active_tile_span_elements=("active_tile_span_elements", "first"),
        ...
        .sort_values(["reuse_radius", "implementation_order", "implementation"])
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    direct = (
        summary[summary["implementation"] == "direct_global"][
            ["reuse_radius", "logical_elements", "gpu_ms_median", "gbps_median", "throughput_median"]
        ]
        .rename(
            columns={
    ...
    relative = direct.merge(tiled, on=["reuse_radius", "logical_elements"], how="inner")
    if relative.empty:
    ...
    return relative.sort_values(["reuse_radius"]).reset_index(drop=True)
...
def _build_stability(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    return stability.sort_values(["reuse_radius", "implementation"]).reset_index(drop=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze Experiment 16 shared memory tiling data.")
    parser.add_argument(
        "--skip-current",
        action="store_true",
    ...
    frame["logical_elements"] = frame["output_elements"]
    ...
    _write_table(summary, TABLES_DIR / "shared_memory_tiling_summary.csv")
    _write_table(relative, TABLES_DIR / "shared_memory_tiling_relative.csv")
    _write_table(stability, TABLES_DIR / "shared_memory_tiling_stability.csv")
    ...
    print(f"[ok] Wrote Experiment 16 analysis tables from {source_label} ({rows} rows, {variants} cases on {gpu_name}).")
