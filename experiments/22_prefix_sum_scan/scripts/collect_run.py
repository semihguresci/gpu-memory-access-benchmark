...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Experiment 22 JSON output into runs/<device>/...")
    parser.add_argument(
        "--input",
        type=Path,
        default=DEFAULT_INPUT,
        help="Input benchmark JSON path (default: experiments/22_prefix_sum_scan/results/tables/benchmark_results.json).",
    )
    parser.add_argument(
        "--runs-dir",
        type=Path,
        default=DEFAULT_RUNS_DIR,
        help="Destination runs directory (default: experiments/22_prefix_sum_scan/runs).",
    )
    parser.add_argument(
        "--label",
        type=str,
    ...
    if not any(str(row.get("experiment_id", "")) == EXPERIMENT_ID for row in rows):
        raise ValueError(
            "Input JSON has rows but no Experiment 22 entries. Run with --experiment 22_prefix_sum_scan first."
        )
