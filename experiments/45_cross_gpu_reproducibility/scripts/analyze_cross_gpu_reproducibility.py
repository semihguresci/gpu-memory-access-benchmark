...
EXPERIMENT_ID = "45_cross_gpu_reproducibility"
...
def _extract_checksum(notes: str) -> int | None:
    match = re.search(r"result_checksum=(\d+)", notes or "")
    if match is None:
        return None
...
def _load_frame(path: Path) -> tuple[pd.DataFrame, dict]:
    ...
    frame["result_checksum"] = frame["notes"].astype(str).map(_extract_checksum)
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"] == "coalesced_reference"][
        ["problem_size", "gpu_ms_median", "throughput_median", "gbps_median"]
    ].rename(
        columns={
    ...
    relative["slowdown_vs_coalesced"] = relative["gpu_ms_median"] / relative["baseline_gpu_ms_median"]
    relative["throughput_ratio_vs_coalesced"] = (
        relative["throughput_median"] / relative["baseline_throughput_median"]
    )
    relative["gbps_ratio_vs_coalesced"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "problem_size",
            "gpu_ms_median",
            "slowdown_vs_coalesced",
            "throughput_median",
            "throughput_ratio_vs_coalesced",
            "gbps_median",
            "gbps_ratio_vs_coalesced",
        ]
    ]


def _build_checksums(frame: pd.DataFrame) -> pd.DataFrame:
    checksums = (
        frame.groupby(["variant", "problem_size"], as_index=False)
        .agg(
            checksum_unique=("result_checksum", "nunique"),
            first_checksum=("result_checksum", "first"),
        )
        .sort_values(["problem_size", "variant"])
        .reset_index(drop=True)
    )
    checksums["checksum_stable"] = checksums["checksum_unique"] == 1
    return checksums


def _build_signature(relative: pd.DataFrame) -> pd.DataFrame:
    max_problem_size = int(relative["problem_size"].max())
    signature = relative[relative["problem_size"] == max_problem_size].copy()
    signature.insert(0, "signature_problem_size", max_problem_size)
    return signature[
        [
            "signature_problem_size",
            "variant",
            "slowdown_vs_coalesced",
            "throughput_ratio_vs_coalesced",
            "gbps_ratio_vs_coalesced",
...
def main() -> None:
    ...
    checksums = _build_checksums(frame)
    signature = _build_signature(relative)
    stability = _build_stability(summary)
    _write_table(summary, "cross_gpu_reproducibility_summary.csv")
    _write_table(relative, "cross_gpu_reproducibility_relative.csv")
    _write_table(checksums, "cross_gpu_reproducibility_checksums.csv")
    _write_table(signature, "cross_gpu_reproducibility_signature.csv")
    _write_table(stability, "cross_gpu_reproducibility_stability.csv")
    print(
