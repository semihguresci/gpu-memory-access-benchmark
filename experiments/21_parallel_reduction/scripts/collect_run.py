...
def main() -> None:
    parser = ArgumentParser(description="Collect Experiment 21 JSON output into runs/<device>/...")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Destination runs directory.",
    )
    parser.add_argument(
        "--label",
        type=str,
    ...
    if not any(str(row.get("experiment_id", "")) == EXPERIMENT_ID for row in rows):
        raise ValueError(
            "Input JSON has rows but no Experiment 21 entries. Run with --experiment 21_parallel_reduction first."
        )
