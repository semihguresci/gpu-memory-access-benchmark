...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Experiment 07 JSON output into runs/<device>/...")
    parser.add_argument(
        "--input",
        type=Path,
        ...
        help=(
            ...
            "(default: experiments/07_aosoa_blocked_layout/results/tables/benchmark_results.json)."
        ),
    )
    parser.add_argument(
        "--runs-dir",
        ...
        help="Destination runs directory (default: experiments/07_aosoa_blocked_layout/runs).",
    )
    parser.add_argument(
        "--label",
        type=str,
    ...
    if not any(str(row.get("experiment_id", "")) == EXPERIMENT_ID for row in rows):
        raise ValueError(
            "Input JSON has rows but no Experiment 07 entries. "
            "Run with --experiment 07_aosoa_blocked_layout first."
        )
