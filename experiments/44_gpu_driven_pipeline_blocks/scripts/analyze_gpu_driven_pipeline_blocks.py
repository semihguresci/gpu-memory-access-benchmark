...
EXPERIMENT_ID = "44_gpu_driven_pipeline_blocks"
...
def _build_summary(frame: pd.DataFrame) -> pd.DataFrame:
    return (
        ...
        .agg(
            ...
            dispatches_per_iteration=("dispatch_count", "median"),
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"] == "staged_three_dispatch"][
    ...
    relative["slowdown_vs_staged"] = relative["gpu_ms_median"] / relative["baseline_gpu_ms_median"]
    relative["throughput_ratio_vs_staged"] = relative["throughput_median"] / relative["baseline_throughput_median"]
    relative["gbps_ratio_vs_staged"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "problem_size",
            ...
            "slowdown_vs_staged",
            "throughput_median",
            "throughput_ratio_vs_staged",
            "gbps_median",
            "gbps_ratio_vs_staged",
        ]
    ]
...
def main() -> None:
    ...
    _write_table(summary, "gpu_driven_pipeline_blocks_summary.csv")
    _write_table(relative, "gpu_driven_pipeline_blocks_relative.csv")
    _write_table(stability, "gpu_driven_pipeline_blocks_stability.csv")
    print(
