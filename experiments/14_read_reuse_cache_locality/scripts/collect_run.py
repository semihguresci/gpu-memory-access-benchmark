...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Experiment 14 JSON output into runs/<device>/...")
    parser.add_argument(
        "--input",
        type=Path,
        ...
        help=(
            ...
            "(default: experiments/14_read_reuse_cache_locality/results/tables/benchmark_results.json)."
        ),
    )
    parser.add_argument(
        "--runs-dir",
        ...
        help="Destination runs directory (default: experiments/14_read_reuse_cache_locality/runs).",
    )
    parser.add_argument(
        "--label",
        type=str,
    ...
    if not any(str(row.get("experiment_id", "")) == EXPERIMENT_ID for row in rows):
        raise ValueError(
            "Input JSON has rows but no Experiment 14 entries. "
            "Run with --experiment 14_read_reuse_cache_locality first."
        )
