...
EXPERIMENT_ID = "39_tiled_light_assignment"
...
def _distribution_from_variant(variant: str) -> str:
    if variant.endswith("_uniform_lights"):
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"].str.startswith("light_atomic_")][
    ...
    relative["slowdown_vs_light_atomic"] = relative["gpu_ms_median"] / relative["baseline_gpu_ms_median"]
    relative["throughput_ratio_vs_light_atomic"] = (
        relative["throughput_median"] / relative["baseline_throughput_median"]
    )
    relative["gbps_ratio_vs_light_atomic"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "distribution",
            "problem_size",
            "gpu_ms_median",
            "slowdown_vs_light_atomic",
            "throughput_median",
            "throughput_ratio_vs_light_atomic",
            "gbps_median",
            "gbps_ratio_vs_light_atomic",
        ]
    ]
...
def main() -> None:
    ...
    _write_table(summary, "tiled_light_assignment_summary.csv")
    _write_table(relative, "tiled_light_assignment_relative.csv")
    _write_table(stability, "tiled_light_assignment_stability.csv")
    print(
