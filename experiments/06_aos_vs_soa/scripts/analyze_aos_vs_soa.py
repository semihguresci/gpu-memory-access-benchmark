...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze Experiment 06 AoS vs SoA runs.")
    parser.add_argument(
        "--runs-dir",
        type=Path,
        default=RUNS_DIR,
        help="Directory containing collected run JSON files (default: experiments/06_aos_vs_soa/runs).",
    )
    parser.add_argument(
        "--skip-current",
        action="store_true",
    ...
    if all_rows.empty or run_index.empty:
        raise FileNotFoundError(
            "No Experiment 06 rows found. Add runs under experiments/06_aos_vs_soa/runs or generate "
    ...
    _write_table(run_index_export, TABLES_DIR / "aos_vs_soa_runs_index.csv")

    summary = _build_summary(all_rows)
    _write_table(summary, TABLES_DIR / "aos_vs_soa_multi_run_summary.csv")
    ...
    _write_table(latest_summary_simple, TABLES_DIR / "aos_vs_soa_summary.csv")

    status = _build_status_overview(latest_rows)
    if not status.empty:
        _write_table(status, TABLES_DIR / "aos_vs_soa_status_overview.csv")

    _plot_metric(
        latest_summary_simple,
        metric_column="gbps_median",
        y_label="GB/s (median)",
        title="Experiment 06 AoS vs SoA: Effective Bandwidth",
        output_path=CHARTS_DIR / "aos_vs_soa_gbps_vs_size.png",
    )
    _plot_metric(
        latest_summary_simple,
        metric_column="gpu_ms_median",
        y_label="gpu_ms (median)",
        title="Experiment 06 AoS vs SoA: Dispatch Time",
        output_path=CHARTS_DIR / "aos_vs_soa_gpu_ms_vs_size.png",
    )
