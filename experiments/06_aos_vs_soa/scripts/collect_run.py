...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Experiment 06 JSON output into runs/<device>/...")
    parser.add_argument(
        "--input",
        type=Path,
        ...
        help=(
            ...
            "(default: experiments/06_aos_vs_soa/results/tables/benchmark_results.json)."
        ),
    )
    parser.add_argument(
        "--runs-dir",
        ...
        help="Destination runs directory (default: experiments/06_aos_vs_soa/runs).",
    )
    parser.add_argument(
        "--label",
        type=str,
    ...
    if not any(str(row.get("experiment_id", "")) == EXPERIMENT_ID for row in rows):
        raise ValueError(
            "Input JSON has rows but no Experiment 06 entries. "
            "Run with --experiment 06_aos_vs_soa first."
        )
