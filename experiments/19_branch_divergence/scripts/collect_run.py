...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Experiment 19 JSON output into runs/<device>/...")
    parser.add_argument(
        "--input",
        type=Path,
        default=DEFAULT_INPUT,
        help="Input benchmark JSON path (default: experiments/19_branch_divergence/results/tables/benchmark_results.json).",
    )
    parser.add_argument(
        "--runs-dir",
        type=Path,
        default=DEFAULT_RUNS_DIR,
        help="Destination runs directory (default: experiments/19_branch_divergence/runs).",
    )
    parser.add_argument(
        "--label",
        type=str,
        default="",
        help="Optional run label appended to the filename.",
    )
    ...
    if not any(str(row.get("experiment_id", "")) == EXPERIMENT_ID for row in rows):
        raise ValueError("Input JSON has rows but no Experiment 19 entries. Run with --experiment 19_branch_divergence first.")
    ...
    try:
        run_id = resolved_output.relative_to(ROOT.resolve()).as_posix()
    except ValueError:
        run_id = resolved_output.as_posix()
