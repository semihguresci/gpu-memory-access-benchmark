...
EXPERIMENT_ID = "33_two_dimensional_locality_transpose_study"
...
def _load_frame(path: Path) -> tuple[pd.DataFrame, dict]:
    ...
    if frame.empty:
        raise ValueError("Input benchmark JSON has no Experiment 33 rows.")
    ...
    frame["matrix_dim"] = pd.to_numeric(note_maps.map(lambda notes: notes.get("matrix_dim", "0")), errors="coerce")
...
def _load_source_frame(skip_current: bool) -> tuple[pd.DataFrame, dict, Path]:
    ...
    if latest is None:
        raise FileNotFoundError("No benchmark_results.json or archived run JSON found for Experiment 33.")
...
def _build_summary(frame: pd.DataFrame) -> pd.DataFrame:
    return (
        frame.groupby(["variant", "matrix_dim", "problem_size"], as_index=False)
        .agg(
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    row_copy = summary[summary["variant"] == "row_major_copy"][["problem_size", "gpu_ms_median"]].rename(
        columns={"gpu_ms_median": "row_copy_gpu_ms_median"}
    )
    naive = summary[summary["variant"] == "naive_transpose"][["problem_size", "gpu_ms_median"]].rename(
        columns={"gpu_ms_median": "naive_gpu_ms_median"}
    )
    relative = summary.merge(row_copy, on="problem_size", how="inner").merge(naive, on="problem_size", how="inner")
    relative["slowdown_vs_row_major_copy"] = relative["gpu_ms_median"] / relative["row_copy_gpu_ms_median"]
    relative["speedup_vs_naive_transpose"] = relative["naive_gpu_ms_median"] / relative["gpu_ms_median"]
    return relative[
        [
            "variant",
            "matrix_dim",
            "problem_size",
            "gpu_ms_median",
            "slowdown_vs_row_major_copy",
            "speedup_vs_naive_transpose",
            "gbps_median",
        ]
    ]
...
def _build_stability(summary: pd.DataFrame) -> pd.DataFrame:
    stability = summary[["variant", "matrix_dim", "problem_size", "gpu_ms_median", "gpu_ms_p95"]].copy()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze Experiment 33 2D locality and transpose data.")
    ...
    _write_table(summary, "two_dimensional_locality_transpose_study_summary.csv")
    _write_table(relative, "two_dimensional_locality_transpose_study_relative.csv")
    _write_table(stability, "two_dimensional_locality_transpose_study_stability.csv")
    print(
        f"[ok] Wrote Experiment 33 analysis tables from {source_path.name} on "
