...
def main() -> None:
    ...
    ax.set_title("Experiment 28: Full GPU-Path Time by Placement")
    ax.set_ylabel("GPU ms (median)")
    ax.grid(True, axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "heap_placement_gpu_path_ms.png", dpi=150)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(summary["variant"], summary["end_to_end_ms_median"], color=["tab:blue", "tab:orange"])
    ax.set_title("Experiment 28: Harness Wall-Clock Time by Placement")
    ax.set_ylabel("Harness wall-clock ms (median)")
    ax.grid(True, axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "heap_placement_harness_wall_clock_ms.png", dpi=150)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(summary["variant"], summary["gbps_median"], color=["tab:green", "tab:red"])
    ax.set_title("Experiment 28: Effective Full GPU-Path GB/s by Placement")
    ax.set_ylabel("GB/s (median)")
    ax.grid(True, axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "heap_placement_gpu_path_gbps.png", dpi=150)
    plt.close(fig)
