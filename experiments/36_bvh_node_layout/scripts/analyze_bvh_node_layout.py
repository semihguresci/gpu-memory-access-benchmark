...
EXPERIMENT_ID = "36_bvh_node_layout"
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"] == "compact32_sequential"][
    ...
    relative["slowdown_vs_compact32"] = relative["gpu_ms_median"] / relative["baseline_gpu_ms_median"]
    relative["gbps_ratio_vs_compact32"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "problem_size",
            "gpu_ms_median",
            "slowdown_vs_compact32",
            "gbps_median",
            "gbps_ratio_vs_compact32",
        ]
    ]
...
def main() -> None:
    ...
    _write_table(summary, "bvh_node_layout_summary.csv")
    _write_table(relative, "bvh_node_layout_relative.csv")
    _write_table(stability, "bvh_node_layout_stability.csv")
    print(
