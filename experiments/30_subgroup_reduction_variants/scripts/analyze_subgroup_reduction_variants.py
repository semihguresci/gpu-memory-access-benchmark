...
EXPERIMENT_ID = "30_subgroup_reduction_variants"
...
def _load_frame(path: Path) -> tuple[pd.DataFrame, dict]:
    ...
    if frame.empty:
        raise ValueError("Input benchmark JSON has no Experiment 30 rows.")
...
def _load_source_frame(skip_current: bool) -> tuple[pd.DataFrame, dict, Path]:
    ...
    if latest is None:
        raise FileNotFoundError("No benchmark_results.json or archived run JSON found for Experiment 30.")
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"] == "shared_tree"][["problem_size", "gpu_ms_median", "gbps_median"]].rename(
    ...
    relative["speedup_vs_shared_tree"] = relative["baseline_gpu_ms_median"] / relative["gpu_ms_median"]
    relative["gbps_ratio_vs_shared_tree"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "problem_size",
            "gpu_ms_median",
            "speedup_vs_shared_tree",
            "gbps_median",
            "gbps_ratio_vs_shared_tree",
        ]
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze Experiment 30 subgroup reduction data.")
    ...
    _write_table(summary, "subgroup_reduction_variants_summary.csv")
    _write_table(relative, "subgroup_reduction_variants_relative.csv")
    _write_table(stability, "subgroup_reduction_variants_stability.csv")
    print(
        f"[ok] Wrote Experiment 30 analysis tables from {source_path.name} on "
