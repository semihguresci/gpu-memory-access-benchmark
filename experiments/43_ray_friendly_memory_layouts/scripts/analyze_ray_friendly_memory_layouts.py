...
EXPERIMENT_ID = "43_ray_friendly_memory_layouts"
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"] == "aos64_sequential"][
    ...
    relative["slowdown_vs_aos64_sequential"] = relative["gpu_ms_median"] / relative["baseline_gpu_ms_median"]
    relative["throughput_ratio_vs_aos64_sequential"] = (
        relative["throughput_median"] / relative["baseline_throughput_median"]
    )
    relative["gbps_ratio_vs_aos64_sequential"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "problem_size",
            "gpu_ms_median",
            "slowdown_vs_aos64_sequential",
            "throughput_median",
            "throughput_ratio_vs_aos64_sequential",
            "gbps_median",
            "gbps_ratio_vs_aos64_sequential",
        ]
    ]
...
def main() -> None:
    ...
    _write_table(summary, "ray_friendly_memory_layouts_summary.csv")
    _write_table(relative, "ray_friendly_memory_layouts_relative.csv")
    _write_table(stability, "ray_friendly_memory_layouts_stability.csv")
    print(
