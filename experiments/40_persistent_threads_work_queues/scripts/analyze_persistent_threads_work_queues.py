...
EXPERIMENT_ID = "40_persistent_threads_work_queues"
...
def _distribution_from_variant(variant: str) -> str:
    if variant.endswith("_uniform_cost"):
        return "uniform_cost"
    if variant.endswith("_skewed_tail"):
        return "skewed_tail"
    return "unknown"
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"].str.startswith("static_partitioned_")][
    ...
    relative["slowdown_vs_static_partitioned"] = relative["gpu_ms_median"] / relative["baseline_gpu_ms_median"]
    relative["throughput_ratio_vs_static_partitioned"] = (
        relative["throughput_median"] / relative["baseline_throughput_median"]
    )
    relative["gbps_ratio_vs_static_partitioned"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "distribution",
            "problem_size",
            "gpu_ms_median",
            "slowdown_vs_static_partitioned",
            "throughput_median",
            "throughput_ratio_vs_static_partitioned",
            "gbps_median",
            "gbps_ratio_vs_static_partitioned",
        ]
    ]
...
def main() -> None:
    ...
    _write_table(summary, "persistent_threads_work_queues_summary.csv")
    _write_table(relative, "persistent_threads_work_queues_relative.csv")
    _write_table(stability, "persistent_threads_work_queues_stability.csv")
    print(
