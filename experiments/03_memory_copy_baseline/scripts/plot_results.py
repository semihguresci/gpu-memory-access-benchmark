...
def _plot_memory_copy_baseline_summary() -> bool:
    ...
    summary = (
        df.groupby(["variant", "problem_size"], as_index=False)
    ...
    fig.suptitle("Experiment 03 Memory Copy Baseline", fontsize=14)
...
def main() -> None:
    if not _plot_memory_copy_baseline_summary():
        raise FileNotFoundError(
            "No Experiment 03 rows found in benchmark_results.json. "
            "Run the benchmark first with --experiment 03_memory_copy_baseline."
        )
