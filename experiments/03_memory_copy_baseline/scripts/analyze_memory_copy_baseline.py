...
EXPERIMENT_ID = "03_memory_copy_baseline"
...
def _build_summary(all_rows: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        .sort_values(["exported_at_utc", "run_id", "variant", "problem_size"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze Experiment 03 memory copy baseline runs.")
    parser.add_argument(
        "--runs-dir",
        type=Path,
        default=RUNS_DIR,
        help="Directory containing collected run JSON files (default: experiments/03_memory_copy_baseline/runs).",
    )
    parser.add_argument(
        "--skip-current",
        action="store_true",
    ...
    if all_rows.empty or run_index.empty:
        raise FileNotFoundError(
            "No Experiment 03 rows found. Add runs under experiments/03_memory_copy_baseline/runs or generate "
    ...
    _write_table(run_index_export, TABLES_DIR / "memory_copy_baseline_runs_index.csv")

    summary = _build_summary(all_rows)
    _write_table(summary, TABLES_DIR / "memory_copy_baseline_multi_run_summary.csv")
    ...
    _write_table(latest_summary_simple, TABLES_DIR / "memory_copy_baseline_summary.csv")

    status = _build_status_overview(latest_rows)
    if not status.empty:
        _write_table(status, TABLES_DIR / "memory_copy_baseline_status_overview.csv")

    _plot_metric(
        latest_summary_simple,
        metric_column="gbps_median",
        y_label="GB/s (median)",
        title="Experiment 03 Memory Copy Baseline: Effective Bandwidth",
        output_path=CHARTS_DIR / "memory_copy_baseline_gbps_vs_size.png",
    )
    _plot_metric(
        latest_summary_simple,
        metric_column="gpu_ms_median",
        y_label="gpu_ms (median)",
        title="Experiment 03 Memory Copy Baseline: Dispatch Time",
        output_path=CHARTS_DIR / "memory_copy_baseline_gpu_ms_vs_size.png",
    )
