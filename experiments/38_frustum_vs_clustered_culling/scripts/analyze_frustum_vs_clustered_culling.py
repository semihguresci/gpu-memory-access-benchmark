...
EXPERIMENT_ID = "38_frustum_vs_clustered_culling"
...
def _distribution_from_variant(variant: str) -> str:
    if variant.endswith("_wide_scene"):
        return "wide_scene"
    if variant.endswith("_center_clustered"):
        return "center_clustered"
    return "unknown"
...
def _build_summary(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    summary["distribution"] = summary["variant"].map(_distribution_from_variant)
    return summary


def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"].str.startswith("frustum_direct_")][
        ["problem_size", "distribution", "gpu_ms_median", "throughput_median", "gbps_median"]
    ].rename(
        columns={
    ...
    relative = summary.merge(baseline, on=["problem_size", "distribution"], how="inner")
    relative["slowdown_vs_frustum_baseline"] = relative["gpu_ms_median"] / relative["baseline_gpu_ms_median"]
    relative["throughput_ratio_vs_frustum_baseline"] = (
        relative["throughput_median"] / relative["baseline_throughput_median"]
    )
    relative["gbps_ratio_vs_frustum_baseline"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "distribution",
            "problem_size",
            "gpu_ms_median",
            "slowdown_vs_frustum_baseline",
            "throughput_median",
            "throughput_ratio_vs_frustum_baseline",
            "gbps_median",
            "gbps_ratio_vs_frustum_baseline",
        ]
    ]


def _build_stability(summary: pd.DataFrame) -> pd.DataFrame:
    stability = summary[["variant", "distribution", "problem_size", "gpu_ms_median", "gpu_ms_p95"]].copy()
...
def main() -> None:
    ...
    _write_table(summary, "frustum_vs_clustered_culling_summary.csv")
    _write_table(relative, "frustum_vs_clustered_culling_relative.csv")
    _write_table(stability, "frustum_vs_clustered_culling_stability.csv")
    print(
