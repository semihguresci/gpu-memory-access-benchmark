...
def main() -> None:
    summary = _load("radix_sort_gpu_summary.csv")
    relative = _load("radix_sort_gpu_relative.csv")
    stability = _load("radix_sort_gpu_stability.csv")
    ...
    ax.set_title("Experiment 34: GPU Time by Problem Size")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("GPU ms (median)")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "radix_sort_gpu_ms.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_title("Experiment 34: Throughput (keys/s) by Problem Size")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("throughput (elements/s, median)")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "radix_sort_throughput.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_title("Experiment 34: Stability by Problem Size")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("p95 / median GPU ms")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "radix_sort_stability.png", dpi=150)
    plt.close(fig)

    print(f"[ok] Wrote Experiment 34 charts to {CHARTS_DIR}.")
