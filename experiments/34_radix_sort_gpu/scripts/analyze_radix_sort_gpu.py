...
EXPERIMENT_ID = "34_radix_sort_gpu"
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"] == "8bit_4pass"][
    ...
    relative["speedup_vs_8bit"] = relative["baseline_gpu_ms_median"] / relative["gpu_ms_median"]
    relative["gbps_ratio_vs_8bit"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "problem_size",
...
def main() -> None:
    ...
    _write_table(summary, "radix_sort_gpu_summary.csv")
    _write_table(relative, "radix_sort_gpu_relative.csv")
    _write_table(stability, "radix_sort_gpu_stability.csv")
    print(
