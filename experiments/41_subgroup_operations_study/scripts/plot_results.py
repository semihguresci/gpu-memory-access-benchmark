...
def main() -> None:
    summary = _load("subgroup_operations_study_summary.csv")
    relative = _load("subgroup_operations_study_relative.csv")
    stability = _load("subgroup_operations_study_stability.csv")
    ...
    ax.set_title("Experiment 41: GPU Time by Element Count")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("GPU ms (median)")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "subgroup_operations_study_ms.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_title("Experiment 41: Throughput by Element Count")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("throughput (elements/s, median)")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "subgroup_operations_study_throughput.png", dpi=150)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(10, 6))
    for variant in relative["variant"].unique():
        subset = relative[relative["variant"] == variant].sort_values("problem_size")
        ax.plot(subset["problem_size"], subset["slowdown_vs_shared_baseline"], marker="o", linewidth=2.0, label=variant)
    ax.axhline(1.0, color="black", linestyle="--", linewidth=1.0)
    ax.set_title("Experiment 41: Slowdown vs shared_baseline")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("slowdown ratio")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "subgroup_operations_study_relative.png", dpi=150)
    plt.close(fig)
    ...
    ax.axhline(1.0, color="black", linestyle="--", linewidth=1.0)
    ax.set_title("Experiment 41: Stability by Element Count")
    ax.set_xlabel("problem size (elements)")
    ax.set_ylabel("p95 / median GPU ms")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHARTS_DIR / "subgroup_operations_study_stability.png", dpi=150)
    plt.close(fig)

    print(f"[ok] Wrote Experiment 41 charts to {CHARTS_DIR}.")
