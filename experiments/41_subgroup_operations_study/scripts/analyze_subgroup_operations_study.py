...
EXPERIMENT_ID = "41_subgroup_operations_study"
...
def _build_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["variant"] == "shared_baseline"][
    ...
    relative["slowdown_vs_shared_baseline"] = relative["gpu_ms_median"] / relative["baseline_gpu_ms_median"]
    relative["throughput_ratio_vs_shared_baseline"] = (
        relative["throughput_median"] / relative["baseline_throughput_median"]
    )
    relative["gbps_ratio_vs_shared_baseline"] = relative["gbps_median"] / relative["baseline_gbps_median"]
    return relative[
        [
            "variant",
            "problem_size",
            "gpu_ms_median",
            "slowdown_vs_shared_baseline",
            "throughput_median",
            "throughput_ratio_vs_shared_baseline",
            "gbps_median",
            "gbps_ratio_vs_shared_baseline",
        ]
    ]
...
def main() -> None:
    ...
    _write_table(summary, "subgroup_operations_study_summary.csv")
    _write_table(relative, "subgroup_operations_study_relative.csv")
    _write_table(stability, "subgroup_operations_study_stability.csv")
    print(
