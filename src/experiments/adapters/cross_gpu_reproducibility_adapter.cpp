...
bool run_cross_gpu_reproducibility_experiment_adapter(VulkanContext& context, const BenchmarkRunner& runner,
                                                      const AppOptions& options, ExperimentRunOutput& output) {
    CrossGpuReproducibilityExperimentOutput experiment_output = run_cross_gpu_reproducibility_experiment(
        context, runner,
        CrossGpuReproducibilityExperimentConfig{
    ...
    if (output.summary_results.empty()) {
        ...
        output.error_message = "cross-gpu reproducibility experiment produced no summary results.";
        return false;
    }

    if (!experiment_output.all_points_correct) {
        output.success = false;
        output.error_message = "cross-gpu reproducibility experiment reported correctness failures.";
        return false;
    }
