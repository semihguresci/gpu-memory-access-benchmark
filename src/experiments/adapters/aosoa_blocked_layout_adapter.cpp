...
bool run_aosoa_blocked_layout_experiment_adapter(VulkanContext& context, const BenchmarkRunner& runner,
    ...
    if (output.summary_results.empty()) {
        ...
        output.error_message = "AoSoA blocked layout experiment produced no summary results.";
        return false;
    }

    if (!experiment_output.all_points_correct) {
        output.success = false;
        output.error_message = "AoSoA blocked layout experiment reported correctness failures.";
        return false;
    }
