...
bool run_bvh_node_layout_experiment_adapter(VulkanContext& context, const BenchmarkRunner& runner,
                                            const AppOptions& options, ExperimentRunOutput& output) {
    BvhNodeLayoutExperimentOutput experiment_output = run_bvh_node_layout_experiment(
        context, runner,
        BvhNodeLayoutExperimentConfig{
    ...
    if (output.summary_results.empty()) {
        ...
        output.error_message = "bvh node layout experiment produced no summary results.";
        return false;
    }

    if (!experiment_output.all_points_correct) {
        output.success = false;
        output.error_message = "bvh node layout experiment reported correctness failures.";
        return false;
    }
