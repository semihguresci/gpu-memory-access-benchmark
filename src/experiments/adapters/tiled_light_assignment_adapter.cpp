...
bool run_tiled_light_assignment_experiment_adapter(VulkanContext& context, const BenchmarkRunner& runner,
                                                   const AppOptions& options, ExperimentRunOutput& output) {
    TiledLightAssignmentExperimentOutput experiment_output = run_tiled_light_assignment_experiment(
        context, runner,
        TiledLightAssignmentExperimentConfig{
    ...
    if (output.summary_results.empty()) {
        ...
        output.error_message = "tiled light assignment experiment produced no summary results.";
        return false;
    }

    if (!experiment_output.all_points_correct) {
        output.success = false;
        output.error_message = "tiled light assignment experiment reported correctness failures.";
        return false;
    }
