...
bool run_register_pressure_proxy_experiment_adapter(VulkanContext& context, const BenchmarkRunner& runner,
                                                    const AppOptions& options, ExperimentRunOutput& output) {
    RegisterPressureProxyExperimentOutput experiment_output = run_register_pressure_proxy_experiment(
        context, runner,
        RegisterPressureProxyExperimentConfig{
    ...
    if (output.summary_results.empty()) {
        ...
        output.error_message = "register pressure proxy experiment produced no summary results.";
        return false;
    }

    if (!experiment_output.all_points_correct) {
        output.success = false;
        output.error_message = "register pressure proxy experiment reported correctness failures.";
        return false;
    }
