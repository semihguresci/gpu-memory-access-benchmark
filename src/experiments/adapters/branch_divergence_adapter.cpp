...
bool run_branch_divergence_experiment_adapter(VulkanContext& context, const BenchmarkRunner& runner,
                                              const AppOptions& options, ExperimentRunOutput& output) {
    BranchDivergenceExperimentOutput experiment_output = run_branch_divergence_experiment(
        context, runner,
        BranchDivergenceExperimentConfig{
    ...
    if (output.summary_results.empty()) {
        ...
        output.error_message = "branch divergence experiment produced no summary results.";
        return false;
    }

    if (!experiment_output.all_points_correct) {
        output.success = false;
        output.error_message = "branch divergence experiment reported correctness failures.";
        return false;
    }
