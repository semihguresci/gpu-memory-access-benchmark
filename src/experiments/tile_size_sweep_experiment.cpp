...
constexpr const char* kExperimentId = "17_tile_size_sweep";
constexpr uint32_t kKernelRadius = 16U;
constexpr uint32_t kMaxTileSize = 64U;
constexpr uint32_t kWorkgroupMultiple = kMaxTileSize;
constexpr uint32_t kCenterOffset = kKernelRadius;
...
constexpr std::array<VariantDescriptor, 8> kVariantDescriptors = {{
    {ImplementationKind::DirectGlobal, 8U, "direct_global", "17_tile_size_sweep_direct_t8.comp.spv"},
    {ImplementationKind::SharedTiled, 8U, "shared_tiled", "17_tile_size_sweep_tiled_t8.comp.spv"},
    {ImplementationKind::DirectGlobal, 16U, "direct_global", "17_tile_size_sweep_direct_t16.comp.spv"},
    {ImplementationKind::SharedTiled, 16U, "shared_tiled", "17_tile_size_sweep_tiled_t16.comp.spv"},
    {ImplementationKind::DirectGlobal, 32U, "direct_global", "17_tile_size_sweep_direct_t32.comp.spv"},
    {ImplementationKind::SharedTiled, 32U, "shared_tiled", "17_tile_size_sweep_tiled_t32.comp.spv"},
    {ImplementationKind::DirectGlobal, 64U, "direct_global", "17_tile_size_sweep_direct_t64.comp.spv"},
    {ImplementationKind::SharedTiled, 64U, "shared_tiled", "17_tile_size_sweep_tiled_t64.comp.spv"},
}};
...
std::string make_variant_name(ImplementationKind kind, uint32_t tile_size) {
    const char* implementation_name = (kind == ImplementationKind::DirectGlobal) ? "direct_global" : "shared_tiled";
    return std::string(implementation_name) + "_t" + std::to_string(tile_size);
}

std::string make_case_name(ImplementationKind kind, uint32_t tile_size, uint32_t logical_count) {
    return std::string(kExperimentId) + "_" + make_variant_name(kind, tile_size) + "_outputs_" +
...
std::vector<uint32_t> build_reference_values(const uint32_t* src_values, uint32_t logical_count) {
    ...
    for (uint32_t logical_index = 0U; logical_index < logical_count; ++logical_index) {
        ...
        uint32_t sum = 0U;
        for (uint32_t offset = 0U; offset < ((2U * kKernelRadius) + 1U); ++offset) {
            sum += src_values[(center_index - kKernelRadius) + offset];
        }
...
CountResolution determine_logical_count(std::size_t max_buffer_bytes, uint32_t max_dispatch_groups_x) {
    ...
    if (source_capacity_elements <= (2U * kKernelRadius)) {
        return resolution;
    }

    const uint64_t source_limited_count = source_capacity_elements - (2U * kKernelRadius);
    const uint64_t output_limited_count = static_cast<uint64_t>(max_buffer_bytes) / sizeof(uint32_t);
    const uint64_t dispatch_limited_count = static_cast<uint64_t>(max_dispatch_groups_x) * kWorkgroupMultiple;
    const uint64_t effective_count = std::min({source_limited_count, output_limited_count, dispatch_limited_count,
                                               static_cast<uint64_t>(std::numeric_limits<uint32_t>::max())});
    if (effective_count < kWorkgroupMultiple) {
        return resolution;
    }

    const uint64_t rounded_count = effective_count - (effective_count % kWorkgroupMultiple);
    if (rounded_count == 0U) {
        return resolution;
    }
...
uint32_t compute_source_padded_elements(uint32_t logical_count) {
    return logical_count + (2U * kKernelRadius);
}
...
uint32_t compute_tile_span_elements(uint32_t tile_size) {
    return tile_size + (2U * kKernelRadius);
}

uint32_t compute_load_rounds(uint32_t tile_size) {
    const uint32_t tile_span = compute_tile_span_elements(tile_size);
    return (tile_span + tile_size - 1U) / tile_size;
}

VkDeviceSize compute_estimated_global_read_bytes(ImplementationKind kind, uint32_t logical_count, uint32_t tile_size,
                                                 uint32_t group_count_x) {
    switch (kind) {
    case ImplementationKind::DirectGlobal:
        return static_cast<VkDeviceSize>(logical_count) * static_cast<VkDeviceSize>((2U * kKernelRadius) + 1U) *
               static_cast<VkDeviceSize>(sizeof(uint32_t));
    case ImplementationKind::SharedTiled:
        return static_cast<VkDeviceSize>(group_count_x) *
               static_cast<VkDeviceSize>(compute_tile_span_elements(tile_size)) *
               static_cast<VkDeviceSize>(sizeof(uint32_t));
    }
...
VkDeviceSize compute_shared_bytes_per_workgroup(ImplementationKind kind, uint32_t tile_size) {
    if (kind != ImplementationKind::SharedTiled) {
        return 0U;
    }

    return static_cast<VkDeviceSize>(compute_tile_span_elements(tile_size)) *
           static_cast<VkDeviceSize>(sizeof(uint32_t));
}
...
bool create_buffer_resources(VulkanContext& context, VkDeviceSize source_bytes, VkDeviceSize output_bytes,
    ...
    if (!create_buffer_resource(
        ...
        std::cerr << "Failed to create tile size sweep source buffer.\n";
        return false;
    }
    ...
    if (!create_buffer_resource(
        ...
        std::cerr << "Failed to create tile size sweep destination buffer.\n";
        destroy_buffer_resource(context.device(), out_resources.src_buffer);
        return false;
    }

    if (!map_buffer_memory(context, out_resources.src_buffer, "tile size sweep source buffer",
    ...
    if (!map_buffer_memory(context, out_resources.dst_buffer, "tile size sweep destination buffer",
...
bool create_pipeline_resources(VulkanContext& context, const std::string& shader_path, const BufferResources& buffers,
    ...
    if (!VulkanComputeUtils::load_shader_module_from_file(context.device(), shader_path, out_resources.shader_module)) {
        std::cerr << "Failed to load tile size sweep shader module: " << shader_path << "\n";
        return false;
    }
    ...
    if (!VulkanComputeUtils::create_descriptor_set_layout(context.device(), bindings,
        ...
        std::cerr << "Failed to create tile size sweep descriptor set layout.\n";
        return false;
    }
    ...
    if (!VulkanComputeUtils::create_descriptor_pool(context.device(), pool_sizes, 1U, out_resources.descriptor_pool)) {
        std::cerr << "Failed to create tile size sweep descriptor pool.\n";
        return false;
    }
    ...
    if (!VulkanComputeUtils::allocate_descriptor_set(context.device(), out_resources.descriptor_pool,
        ...
        std::cerr << "Failed to allocate tile size sweep descriptor set.\n";
        return false;
    }
    ...
    if (!VulkanComputeUtils::create_pipeline_layout(context.device(), {out_resources.descriptor_set_layout},
        ...
        std::cerr << "Failed to create tile size sweep pipeline layout.\n";
        return false;
    }
    ...
    if (!VulkanComputeUtils::create_compute_pipeline(context.device(), out_resources.shader_module,
        ...
        std::cerr << "Failed to create tile size sweep compute pipeline.\n";
        return false;
    }
...
double run_dispatch(VulkanContext& context, const PipelineResources& resources, uint32_t logical_count,
                    uint32_t tile_size) {
    const uint32_t group_count_x = VulkanComputeUtils::compute_group_count_1d(logical_count, tile_size);
    if (group_count_x == 0U) {
...
void record_case_notes(std::string& notes, ImplementationKind kind, uint32_t tile_size, uint32_t logical_count,
    ...
    const uint32_t tile_span = compute_tile_span_elements(tile_size);
    const uint32_t load_rounds = compute_load_rounds(tile_size);
    const VkDeviceSize estimated_global_read_bytes =
        compute_estimated_global_read_bytes(kind, logical_count, tile_size, group_count_x);
    ...
    append_note(notes, "tile_size=" + std::to_string(tile_size));
    append_note(notes, "radius=" + std::to_string(kKernelRadius));
    append_note(notes, "center_offset=" + std::to_string(kCenterOffset));
    append_note(notes, "local_size_x=" + std::to_string(tile_size));
    append_note(notes, "group_count_x=" + std::to_string(group_count_x));
    append_note(notes, "tile_span_elements=" + std::to_string(tile_span));
    append_note(notes, "load_rounds=" + std::to_string(load_rounds));
    append_note(notes, "shared_bytes_per_workgroup=" + std::to_string(static_cast<unsigned long long>(
                                                           compute_shared_bytes_per_workgroup(kind, tile_size))));
    ...
    append_note(notes, "occupancy_proxy=local_size_x");
...
bool run_case(VulkanContext& context, const BenchmarkRunner& runner, const BufferResources& buffers,
              const PipelineResources& pipeline_resources, ImplementationKind kind, uint32_t tile_size,
              uint32_t logical_count, bool rounded_to_workgroup_multiple, const std::vector<uint32_t>& reference_values,
              TileSizeSweepExperimentOutput& output, bool verbose_progress, std::size_t scratch_size_bytes) {
    const std::string variant_name = make_variant_name(kind, tile_size);
    const uint32_t group_count_x = VulkanComputeUtils::compute_group_count_1d(logical_count, tile_size);
    ...
    for (int warmup = 0; warmup < runner.warmup_iterations(); ++warmup) {
        ...
        const double dispatch_ms = run_dispatch(context, pipeline_resources, logical_count, tile_size);
        ...
        if (!dispatch_ok || !data_ok) {
            std::cerr << "[" << kExperimentId << "] Warmup issue for variant=" << variant_name
                      << ", logical_outputs=" << logical_count << ", tile_size=" << tile_size
    ...
    for (int iteration = 0; iteration < runner.timed_iterations(); ++iteration) {
        ...
        std::string notes;
        record_case_notes(notes, kind, tile_size, logical_count, group_count_x, rounded_to_workgroup_multiple,
        ...
        output.rows.push_back(BenchmarkMeasurementRow{
            ...
            .end_to_end_ms = end_to_end_ms.count(),
            .throughput =
                ExperimentMetrics::compute_throughput_elements_per_second(logical_count, kDispatchCount, dispatch_ms),
    ...
    const BenchmarkResult summary =
        BenchmarkRunner::summarize_samples(make_case_name(kind, tile_size, logical_count), dispatch_samples);
...
TileSizeSweepExperimentOutput run_tile_size_sweep_experiment(VulkanContext& context, const BenchmarkRunner& runner,
    ...
    TileSizeSweepExperimentOutput output{};

    if (!context.gpu_timestamps_supported()) {
        std::cerr << "tile size sweep experiment requires GPU timestamp support.\n";
    ...
    for (std::size_t index = 0; index < kVariantDescriptors.size(); ++index) {
        ...
        if (shader_paths[index].empty()) {
            std::cerr << "Could not locate SPIR-V shader for tile size sweep variant " << descriptor.implementation_name
                      << "_t" << descriptor.tile_size << ".\n";
            output.all_points_correct = false;
            return output;
        }
    }
    ...
    if (count_resolution.logical_count == 0U) {
        std::cerr << "Scratch buffer too small for tile size sweep experiment.\n";
        output.all_points_correct = false;
        return output;
    }
    ...
    const std::vector<uint32_t> reference_values = build_reference_values(src_values, logical_count);
    for (std::size_t index = 0; index < kVariantDescriptors.size(); ++index) {
        const auto& descriptor = kVariantDescriptors[index];
        if (!run_case(context, runner, buffers, pipeline_resources[index], descriptor.kind, descriptor.tile_size,
                      logical_count, count_resolution.rounded_to_workgroup_multiple, reference_values, output,
