...
bool create_experiment_buffer_resources(VulkanContext& context, VkDeviceSize max_buffer_size,
    ...
    if (!create_buffer_resource(context.physical_device(), context.device(), max_buffer_size,
        ...
        std::cerr << "Failed to create source device buffer for memory copy baseline experiment.\n";
        return false;
    }
    ...
    if (!create_buffer_resource(context.physical_device(), context.device(), max_buffer_size,
        ...
        std::cerr << "Failed to create destination device buffer for memory copy baseline experiment.\n";
        return false;
    }
    ...
    if (!create_buffer_resource(context.physical_device(), context.device(), max_buffer_size,
        ...
        std::cerr << "Failed to create staging buffer for memory copy baseline experiment.\n";
        return false;
    }
...
std::vector<uint32_t> make_problem_sizes(uint32_t max_elements, uint32_t max_group_count_x) {
    ...
    for (uint32_t power = kMinBytesPower; power <= kMaxBytesPower; ++power) {
        ...
        const uint64_t elements_u64 = bytes / sizeof(float);
...
run_memory_copy_baseline_experiment(VulkanContext& context, const BenchmarkRunner& runner,
    ...
    const std::string read_only_shader_path =
        VulkanComputeUtils::resolve_shader_path(config.read_only_shader_path, "03_memory_read_only.comp.spv");
    if (read_only_shader_path.empty()) {
        std::cerr << "Could not locate SPIR-V shader for memory copy baseline read-only mode.\n";
        return output;
    }

    const std::string write_only_shader_path =
        VulkanComputeUtils::resolve_shader_path(config.write_only_shader_path, "03_memory_write_only.comp.spv");
    if (write_only_shader_path.empty()) {
        std::cerr << "Could not locate SPIR-V shader for memory copy baseline write-only mode.\n";
        return output;
    }

    const std::string read_write_copy_shader_path = VulkanComputeUtils::resolve_shader_path(
        config.read_write_copy_shader_path, "03_memory_read_write_copy.comp.spv");
    if (read_write_copy_shader_path.empty()) {
        std::cerr << "Could not locate SPIR-V shader for memory copy baseline read+write copy mode.\n";
        return output;
    }
    ...
    if (buffer_limited_elements_u64 == 0U) {
        std::cerr << "Scratch buffer too small for memory copy baseline experiment.\n";
        return output;
    }
    ...
    if (effective_max_elements_u64 == 0U) {
        std::cerr << "No legal problem sizes available for memory copy baseline experiment.\n";
        return output;
    }

    if (effective_max_elements_u64 < buffer_limited_elements_u64) {
        std::cerr << "Memory copy baseline sweep clamped by maxComputeWorkGroupCount[0].\n";
    }
    ...
    if (problem_sizes.empty()) {
        std::cerr << "Scratch buffer is too small for the planned [1 MiB, 1 GiB] sweep.\n";
        return output;
    }
    ...
    if (map_result != VK_SUCCESS || mapped_staging == nullptr) {
        std::cerr << "vkMapMemory failed for memory copy baseline staging buffer with error code " << map_result
                  << "\n";
        destroy_variant_pipeline_resources(context, copy_resources);
        destroy_variant_pipeline_resources(context, write_only_resources);
        destroy_variant_pipeline_resources(context, read_only_resources);
