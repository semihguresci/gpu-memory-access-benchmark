...
from experiment_workflow import enabled_experiment_ids, load_enabled_experiment_configs


EXPERIMENT_CONFIG = load_enabled_experiment_configs()
EXPERIMENT_IDS = list(enabled_experiment_ids())
...
def generate_experiment(experiment_id: str, collect_run: bool) -> bool:
    ...
    benchmark_json = config.output_path
    ...
    if collect_run and benchmark_json.exists():
        run_command([sys.executable, str(config.collect_script)])
    ...
    if benchmark_json.exists():
        ...
        if config.supports_skip_current:
    ...
    if config.has_plot_script:
        ...
        if benchmark_json.exists() or config.plot_after_skip_current:
            run_command([sys.executable, str(config.plot_script)])
        else:
